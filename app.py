...
with st.sidebar:
    title = st.text_input(label='Title', max_chars=200, placeholder='what do you want?')
    description = st.text_area(label='Description', placeholder='write any details ...')
...
for task in all_tasks:
    if task['completed'] and not show_completed:
        continue
    with st.expander(f"[ {task["id"]} ] {task['title']}", icon=f'{'👌' if task['completed'] else '❄️'}'):
        col1, col2, col3 = st.columns([3,1,1])
        with col1: 
            description = st.write(f"{task['description'] if task['description'] else "nothing special."}")
        with col2:
            task_checkbox = st.button(f"{"Done!" if not task["completed"] else "Not done yet!"}", key=f'toggle_task_{task['id']}')
            if task_checkbox:
                update_request = rq.patch(url=f"{getenv('API_URL')}{task['id']}",json={"completed": not task['completed']})
