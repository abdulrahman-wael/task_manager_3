...
@app.get('/tasks/')
async def read_all_tasks(session:sessionDep, limit=Query(default=100, le=100), offset:int=0)->list[Task]:
...
@app.patch('/tasks/{id}')
async def update_one_task(session:sessionDep, id:int, changed_fields:UpdateTask) ->Task:
    task = session.get(Task, id)
    if not task:
        raise HTTPException(status_code=404 , detail=f"task id {id} is not available")
    dump = changed_fields.model_dump(exclude_unset=True) # didn't add "exclude_unset" from the beginning
